...
class CLS_TEMPLATE{
	...
	function Load_lang_default(){
		...
		include_once(LAG_PATH.CURENT_LANG."/english.php");
	}
	function WapperTem(){
		if($this->error())
			return;
		include_once(THIS_TEM_PATH."home.php");
	}
	// Test template
	function error(){
	...
	function loadModule($position,$site="site"){
		...
		{
		// kiem tra module có cho phép hiển thị ở cur_menu không
		$mnus=$rows["mnuids"];
		if(trim($rows["mnuids"])=="")
		continue;
		if($rows["mnuids"]!="all"){
	...
	function loadComonent()
	{
		$com="";
		if(isset($_GET["com"]))
		$com=$_GET["com"];
		
		$sql="SELECT * FROM tbl_component WHERE `code`='$com' AND `isactive`='1'";
		$objdata=new CLS_MYSQL;
		...
		if ($objdata->Numrows()>0)
		{
			...
			if(is_dir(COM_PATH."com_".$rows["code"]))
			{	
				define("DIR_URLCOM",COM_PATH."com_".$rows["code"]);
				include_once(COM_PATH."com_".$rows["code"]."/layout.php");
			}
		}
	}
	/*-------------------------------------------------------*/
