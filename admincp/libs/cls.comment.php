...
class CLS_COMM{
	...
	function listTableCommentNews($strwhere="",$page){
		...
		$leng=MAX_ROWS;
		$sql="SELECT * FROM `view_comments` ".$strwhere ." order by `comm_id` desc LIMIT $star,$leng";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
		$i=0;
	...
	function Publish($comm_id){
		$sql="UPDATE `view_comments` SET `isactive` = \"1\" WHERE `comm_id` in ('$comm_id')";
		$objdata=new CLS_MYSQL();
		return $objdata->Query($sql);
	}
	function UnPublish($comm_id){
		$sql="UPDATE `view_comments` SET `isactive` = \"0\" WHERE `comm_id` in ('$comm_id')";
		$objdata=new CLS_MYSQL();
		return $objdata->Query($sql);
	}
