...
class CLS_MEMBER extends CLS_MYSQL{
	...
	function getMemberByID($memid){
		...
		if($objdata->Numrows()>0)
		{
			...
			$this->pro_mem["par_id"]=$rows["par_id"];
		}
	}
	...
	function listTableMember($strwhere="",$page){
		...
		{	$i++;
			...
			echo "<td width=\"50\" align=\"center\">";
			//
			$sql2 = "SELECT * FROM `tbl_member` WHERE `par_id` = '$memid'";
			$objdata2->query($sql2);
			if($objdata2->Numrows()<=0)
				echo "<a href=\"index.php?com=".COMS."&amp;task=delete&amp;memid=$memid\" onclick=\"return confirm('".CF_DELETE01."');\">";
			else 
				echo "<a href=\"index.php?com=".COMS."&amp;task=delete&amp;mnuid=$memid\" onclick=\"return confirm('".CF_PR_DELETE01."');\">";
			//
	...
	function Add_new(){
		$sql="INSERT INTO `tbl_member`(`username`,`password`,`firstname`,`lastname`,`birthday`,`gender`,`address`,`location`,`phone`,`mobile`,`email`,`joindate`,`gmember`,`isactive`,`par_id`) VALUES ";
	...
	function Delete($memid){
		$objdata=new CLS_MYSQL();
		$sql="DELETE FROM `tbl_member` WHERE `par_id` in ('$memid')";
		$objdata->Query($sql);
